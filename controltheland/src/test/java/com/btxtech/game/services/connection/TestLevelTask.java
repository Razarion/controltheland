...
public class TestLevelTask extends AbstractServiceTest {
    ...
    public void realGameInfo() throws Exception {
        ...
        LevelTaskPacket levelTaskPacket = getMovableService().getRealGameInfo().getLevelTaskPacket();
        Assert.assertEquals(new QuestInfo(TEST_LEVEL_TASK_1_2_REAL_NAME, "Descr2", 100, 10, TEST_LEVEL_TASK_1_2_REAL_ID, QuestInfo.Type.QUEST), levelTaskPacket.getQuestInfo());
        ...
        levelTaskPacket = getMovableService().getRealGameInfo().getLevelTaskPacket();
        Assert.assertEquals(new QuestInfo(TEST_LEVEL_TASK_2_2_REAL_NAME, "Descr222", 120, 80, TEST_LEVEL_TASK_2_2_REAL_ID, QuestInfo.Type.QUEST), levelTaskPacket.getQuestInfo());
        Assert.assertFalse(levelTaskPacket.isCompleted());
        ...
        levelTaskPacket = getMovableService().getRealGameInfo().getLevelTaskPacket();
        Assert.assertEquals(new QuestInfo("Task3Level2", "DecrTask3Level2", 100, 10, TEST_LEVEL_TASK_1_3_REAL_ID, QuestInfo.Type.QUEST), levelTaskPacket.getQuestInfo());
        userGuidanceService.activateQuest(TEST_LEVEL_TASK_3_3_SIMULATED_ID);
        levelTaskPacket = getMovableService().getRealGameInfo().getLevelTaskPacket();
        Assert.assertEquals(new QuestInfo(TEST_LEVEL_TASK_3_3_SIMULATED_NAME, "Task3Level2Descr", 2, 0, TEST_LEVEL_TASK_3_3_SIMULATED_ID, QuestInfo.Type.MISSION), levelTaskPacket.getQuestInfo());
        // No quests / missions
    ...
    public void packetSending() throws Exception {
        ...
        Assert.assertEquals(new QuestInfo(TEST_LEVEL_TASK_2_2_REAL_NAME, "Descr222", 120, 80, TEST_LEVEL_TASK_2_2_REAL_ID, QuestInfo.Type.QUEST), levelTaskPackets.get(1).getQuestInfo());
        Assert.assertEquals("build 0", levelTaskPackets.get(1).getActiveQuestProgress());
        userGuidanceService.promote(userService.getUserState(), TEST_LEVEL_3_REAL_ID);
        levelTaskPackets = getPackages(LevelTaskPacket.class);
        Assert.assertEquals(1, levelTaskPackets.size());
        Assert.assertFalse(levelTaskPackets.get(0).isCompleted());
        Assert.assertEquals(new QuestInfo("Task3Level2", "DecrTask3Level2", 100, 10, TEST_LEVEL_TASK_1_3_REAL_ID, QuestInfo.Type.QUEST), levelTaskPackets.get(0).getQuestInfo());
        Assert.assertEquals("", levelTaskPackets.get(0).getActiveQuestProgress());
        userGuidanceService.activateQuest(TEST_LEVEL_TASK_3_3_SIMULATED_ID);
        levelTaskPackets = getPackages(LevelTaskPacket.class);
        Assert.assertEquals(1, levelTaskPackets.size());
        Assert.assertFalse(levelTaskPackets.get(0).isCompleted());
        Assert.assertEquals(new QuestInfo(TEST_LEVEL_TASK_3_3_SIMULATED_NAME, "Task3Level2Descr", 2, 0, TEST_LEVEL_TASK_3_3_SIMULATED_ID, QuestInfo.Type.MISSION), levelTaskPackets.get(0).getQuestInfo());
        Assert.assertNull(levelTaskPackets.get(0).getActiveQuestProgress());
        // No quests / missions
        userGuidanceService.promote(userService.getUserState(), TEST_LEVEL_4_REAL_ID);
        userGuidanceServiceImpl.conditionPassed(getUserState(), TEST_LEVEL_TASK_1_4_REAL_ID);

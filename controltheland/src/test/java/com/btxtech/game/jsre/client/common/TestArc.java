...
public class TestArc {
    ...
    @Test
    public void isNextPointOnSegment() {
        Index middle = new Index(300, 300);
        Arc arc = new Arc(new Index(300, 500), new Index(500, 300), middle);
        Index cross = middle.getPointFromAngelToNord(MathHelper.gradToRad(225), (int) Math.round(arc.getRadius()));

        Assert.assertTrue(arc.isNextPointOnSegment(cross, 100, middle, true));
        Assert.assertTrue(arc.isNextPointOnSegment(cross, 100, middle, false));
        Assert.assertFalse(arc.isNextPointOnSegment(cross, 200, middle, true));
        Assert.assertFalse(arc.isNextPointOnSegment(cross, 201, middle, false));

        arc = new Arc(new Index(740, 460), new Index(540, 260), new Index(540, 460));
        cross = new Index(740, 460);
        Index reference = new Index(500, 500);
        Assert.assertTrue(arc.isNextPointOnSegment(cross, 17, reference, true));
        Assert.assertFalse(arc.isNextPointOnSegment(cross, 17, reference, false));

        arc = new Arc(new Index(1717, 1289), new Index(1438, 1248), new Index(1557, 1408));
        cross = new Index(1657, 1236);
        reference = new Index(1500, 1500);
        Assert.assertFalse(arc.isNextPointOnSegment(cross, 82, reference, false));
        Assert.assertTrue(arc.isNextPointOnSegment(cross, 82, reference, true));
    }
    ...
    public void getNextPoint() {
        ...
        Assert.assertEquals(new Index(455, 427), arc.getNextPoint(cross, 20, null, true));
        Assert.assertEquals(new Index(427, 455), arc.getNextPoint(cross, 20, null, false));
    }
